		...
		-- * Might wanna use their numerical codes instead of named codes for reduced file size (Could be an Option)
			...
			for rangeStart, rangeEnd in string.gmatch(ESCAPES_PATTERN, "(.)%-(.)") do
				for charCode = string.byte(rangeStart), string.byte(rangeEnd) do
					ESCAPES[string.character(charCode)] = "&#" .. charCode .. ";"
				end
			end
...
-- Complex condition: local EXECUTOR_NAME = identify_executor and identify_executor() or ""
...
-- Complex condition: -- * Tests if base64encode exists and works properly then benchmark it
...
-- Complex condition: identifier = 1e15, -- 1 quadrillion, up to 9.(9) quadrillion, in theory this shouldn't ever run out and be enough for all sharedstrings ever imaginable
											identifier = 1e15, -- 1 quadrillion, up to 9.(9) quadrillion, in theory this shouldn't ever run out and be enough for all sharedstrings ever imaginable
												-- TODO: worst case, add fallback to str randomizer once numbers run out : )
													...
													["CFrame"] = function(raw)
														...
														-- local position = write_vector3(raw.Position)
																																	...
																																	-- But it seems like even if those are present, Studio still opens the file just fine
																																		-- So there is no need to check for them currently
																																			...
																																			-- ? Return early for empty tags (this proved equally as fast when done using counter / next)
																																				...
																																				for attr, value in next, attrs do
																																					attrs_n = attrs_n + 1
																																					...
																																					attrs_formatted[attr], attr_size = Descriptor(value)
																																					...
																																					MaterialColors = function(instance) -- https://github.com / RobloxAPI / spec / blob / master / properties / MaterialColors.md
																																						...
																																						{ --https://github.com / rojo - rbx / rbx - dom / blob / master / rbx_dom_lua / src / customProperties.lua#L5
																																							...
																																							WorldPivotData = "WorldPivot", -- TODO This doesn't accurately represent whether optional type property is present or not (it's never nil), gethiddenproperty or gethiddenproperty_fallback is preferred
																																								...
																																								}, -- ? Only needed for execs that lack LocalUserSecurity (Level 2, 5, 9)
...
-- Complex condition: if 649 <= CLIENT_VERSION and ValueType_Name == "Content" then -- TODO: Remove after Roblox adds a descriptor for it
																																																							...
																																																							--- @field decomptype string -- * "custom" - for built - in custom decompiler. ___Default:___ Your executor's decompiler, if available. Otherwise uses "custom" if not.
																																																								...
																																																								--- @field SaveBytecode boolean -- Includes bytecode in the output. Useful if you wish to be able to decompile it yourself later. ___Default:___ false
																																																									...
																																																									--- .IgnoreList {Instance | Instance.ClassName | [Instance.ClassName] = {Instance.Name}} -- Structure is similar to **@DecompileIgnore** except `= false` meaning if you ignore one instance it will automatically ignore it's descendants. ___Default:___ {CoreGui, CorePackages}
																																																										...
																																																										--- @field Object Instance -- * If provided, saves as .rbxmx (Model file) instead. If Object is game, it will be saved as a .rbxl file. **MUST BE AN INSTANCE REFERENCE, FOR EXAMPLE - *Workspace***. `"optimized"` mode is **NOT** supported with this option. If IsModel is set to false then Object specified here will be saved as a place file. Only saves the instance itself, not the descendants. If you wish to save descendants too then use @ExtraInstances={Object}. ___Default:___ false
...
-- Complex condition: @param Parameter_2 table -- [OPTIONAL] If present, then Parameter_2 will be assumed to be [SynSaveInstance.CustomOptions table]. And then if the Parameter_1 is an Instance, then it will be assumed to be [SynSaveInstance.CustomOptions table].Object. If Parameter_1 is a table filled with instances ({Instance}), then it will be assumed to be [SynSaveInstance.CustomOptions table].ExtraInstances and IsModel will be true). This exists for sake compatibility with `saveinstance(game, {})`
																																																																										...
																																																																										local function get_size_format()
																																																																											...
																																																																											local function run_with_loading(text, keepStatus, waitForRender, taskFunction, ...)
																																																																												...
																																																																												if StatusText then
																																																																													...
																																																																													Loading = task.task.task.spawn(function()
																																																																															...
																																																																															elseif Decompiler then
																																																																																...
																																																																																ldecompile = function(script)
																																																																																	...
																																																																																	if ScriptCache then
																																																																																		...
																																																																																		task.task.task.wait() -- TODO Maybe remove?
																																																																																	end
...
-- Complex condition: if o and r.errors then
																																																																																...
																																																																																local function save_cache(final)
																																																																																	...
																																																																																	-- appendfile(placename, savestr) -- * supposedly causes uneven amount of Tags (e.g. <Item> must be closed with </Item> but sometimes there's more of one than the other). While being under load, the function produces unexpected output?
																																																																																		...
																																																																																		local function save_specific(className, properties)
																																																																																			...
																																																																																			for propertyName, value in next, properties do
																																																																																				...
																																																																																				elseif "Name" == propertyName then
																																																																																					whitelisted = true
																																																																																					value, tag = ReturnValueAndTag(value, "string") -- * Doubt ValueType will change
																																																																																				end
...
-- Complex condition: if OnIgnoredList and (OnIgnoredList == true or OnIgnoredList[InstanceName]) then
...
-- Complex condition: if IgnoreSharedStrings and ValueType == "SharedString" then -- ? More info in Options
...
-- Complex condition: if result == nil and unfilterPropVal(Category, Optional) then
...
-- Complex condition: if SharedStringOverwrite and ValueType == "BinaryString" then -- TODO: Convert this to table if more types are added
...
-- Complex condition: if DecompileIgnoring and DecompileIgnoring == instance then
																																																																															...
																																																																															Also http can be converted to https but not sure if Roblox would decide to detect that
																																																																																...
																																																																																-- TODO Find a better solution for this
																																																																																	...
																																																																																	local function applyGameFixes()
																																																																																		...
																																																																																		pcall(function()
																																																																																			game:GetService("TextChatService"):ClearAllChildren()
																																																																																		end)
																																																																																	end
...
-- Complex condition: if ClassTags and ClassTags.Service then -- For CSGDictionaryService, NonReplicatedCSGDictionaryService, LogService, ProximityPromptService, TestService & more
																																																																																	...
																																																																																	if OPTIONS.ReadMe then
																																																																																		...
																																																																																		"Script",
																																																																																		"--[[\n"
																																																																																		.. (RecoveredScripts and "RECOVERED SCRIPTS: " .. service.HttpService:JSONEncode(RecoveredScripts) .. "\n\n" or "")
																																																																																		.. "Game saved successfully!\n\n"
																																																																																		.. "AUTO - FIXES APPLIED:\n"
																																																																																		.. "• ✓ CharacterAutoLoads enabled for proper spawning\n"
																																																																																			.. "• ✓ Workspace spawn properties preserved\n"
																																																																																			.. "• ✓ Camera settings optimized\n"
																																																																																			.. "• ✓ Collision fidelity fixes applied\n\n"
																																																																																			.. "MANUAL FIXES (if needed):\n"
																																																																																				.. "• Chat issues? Run: game:GetService('Chat'):ClearAllChildren()\n"
																																																																																				.. "• Still can't spawn? Check for missing SpawnLocation in workspace\n\n"
																																																																																					.. "Saved in " .. string.format("%.2f", os.clock() - elapse_t) .. "s"
																																																																																					.. " | PlaceId: " .. game.PlaceId
-- Complex condition: .. " | Executor: " .. (identify_executor and table.concat({ identify_executor() }, " ") or "Unknown")
...
-- Complex condition: if IsolateLocalPlayer and IgnoreList.Player ~= true then
																																																																																					...
																																																																																					local function randomString()
																																																																																						local length = math.random(10, 20)
																																																																																							local randomarray = table.create(length)
																																																																																								for i = 1, length do
																																																																																									randomarray[i] = string.character(math.random(32, 126))
																																																																																									end
																																																																																										...
																																																																																										if StatusText then
																																																																																											task.task.task.spawn(function()
																																																																																												...
																																																																																												if ok then
																																																																																													...
																																																																																													task.task.wait(Log10 * 2 + ExtraTime)
																																																																																												else
																																																																																												if Loading then
																																																																																													task.cancel(Loading)
																																																																																													Loading = nil
																																																																																												...
																																																																																												StatusText.Text = "Failed! Check F9 console for more info"
																																																																																													...
																																																																																													warn("Error found while saving:")
																																																																																														warn(err)
																																																																																														task.task.wait(Log10 + ExtraTime)
																																																																																													end
																																																																																													StatusText:Destroy()
																																																																																												end)
																																																																																											end
